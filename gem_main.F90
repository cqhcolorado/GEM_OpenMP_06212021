...
#if  defined(COUPLING) || defined(XGC_3D_OUTPUT)
...
use mapping
  ...
  if(myid==0)write(*,*)'after initialize'
...
#if defined(COUPLING) || defined(XGC_3D_OUTPUT)
call mpi_barrier(mpi_comm_world,ierr) 
if(myid==0)write(*,*)'before cce'
call cce_initialize
call mpi_barrier(mpi_comm_world,ierr)
if(myid==0)write(*,*)'after cce before mapping'
!write(*,*)'before mapping_init,myid=',myid 
call mapping_init
!write(*,*)'after mapping_init,myid=',myid,'gx_id=',gx_id
call mpi_barrier(mpi_comm_world,ierr)
if(myid==0)write(*,*)'after mapping'
#endif
   
  do  timestep=ncurr,nm
     tcurr = tcurr+dt

     !if(myid==0)write(*,*)'point 1'
     !call mpi_barrier(mpi_comm_world,ierr)
     call accumulate(timestep-1,0)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 2'
     call ampere(timestep-1,0)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 3'
     call poisson(timestep-1,0)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 4'
     call field(timestep-1,0)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 5'
     call split_weight(timestep-1,0)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 6'
     call diagnose(timestep-1)
     !call mpi_barrier(mpi_comm_world,ierr)
    !if(myid==0)write(*,*)'point 7'
     call reporter(timestep-1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 8'

     call push_wrapper(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 9'

     call accumulate(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 10'
     call ampere(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 11'
     call poisson(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 12'
      !call mpi_barrier(mpi_comm_world,ierr)
     call field(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 13'
     call split_weight(timestep,1)
      !call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 14'

     call push_wrapper(timestep,0)
      call mpi_barrier(mpi_comm_world,ierr)
     !if(myid==0)write(*,*)'point 15'
...
subroutine init
  ...
  INTEGER :: i,j,k,m,n,ns,idum,i1,j1,k1,ierr
  INTEGER :: mm1,mm2,lr1
...
!$omp target enter data map(to:nonlin)
#endif
  ...
  if((mod(numprocs,kmx) .ne. 0) .and. myid==0)then
    ...
    write(*,*) 'numprocs = ', numprocs, ' kmx = ', kmx
    stop
  endif

  !mm1=int(imx,8)*int(jmx,8)*int(kmx,8)*int(micell,8)
  mm1=int(real(imx*jmx)/real(ntube)*real(micell))
  !mm2=int(imx,8)*int(jmx,8)*int(kmx,8)*int(mecell,8)
  mm2=int(real(imx*jmx)/real(ntube)*real(mecell))
  mmx=mm1*6
  mmxe=mm2*6
  ...
  if(myid==0)write(*,*)'total MPI=',numprocs,' ntube=',ntube,' particle per MPI=',mm1,mm2
  ...
  addi=0.
  if(myid==0)then 
    open(gemout,file='gem.out',form='formatted',status='replace')
  endif
...
!  !$acc update device(apar,dpdz,delby,delbx,dadz)
  !$omp target update to(apar,dpdz,delby,delbx,dadz)
...
subroutine ppush(n,ns)
  ...
  use gem_com
  use gem_equil
  use omp_module,only:nthreads,split_indices
#ifdef COUPLING
  ...
  integer :: mynopi
  integer :: ithread,ith, i_beg(nthreads), i_end(nthreads)
#ifdef DIRECT_F
  real :: new_f0,w4,dw
#endif
  real :: alpha
  ...
  nopi(ns) = 0
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'ppush 1',MPI_WTIME()
call split_indices(mm(ns), nthreads, i_beg, i_end)
#ifdef OPENMP
!!$omp parallel do private( &
!!$omp shared(mm,ns,x2,z2,thfnz,rin,pi,ntheta,dbdr,dbdth,grcgt,bfld,radius,dydr,qhat,gr,gxdgy,curvbz) &
!!$omp shared(bdcrvb,grdgt,f,jfn,psip,lx,delz,dr,dth,psi,t0s,capts,capns,xn0s,lr0) &
!!$omp shared(psip2,dipdr,tor,mims,mu,q0,u2,iflr,lr,y2,ly,dx,dy,dz,q,pi2,gclr,nonlin,delbx,delby) &
!!$omp shared(ez,ex,ey,dpdz,dadz,apar,iorb,r0,br0,kcnt,x3,y3,z3,u3,w2,ildu,lz,nr) &
!!$omp shared(sf,w3,iperidf,w0,pif0,w5,dt,tgis,peritr,vparsp) &
!$omp parallel do private(wz0,wz1,wx0,wx1,th,dbdrp,dbdtp,grcgtp,bfldp,radiusp,dydrp,qhatp,grp,gxdgyp, &
!$omp& curvbzp,bdcrvbp,grdgtp,fp,jfnp,psipp,k,i,psp,kaptp,kapnp,xnp,phincp, &
!$omp& rhox,rhoy,phip,exp1,eyp,ezp,delbxp,delbyp,dpdzp,r,vncp,vparspp,psip2p,dipdrp, &
!$omp& b,dadzp,aparp,xs,xt,wk0,wi1,wi0,wj1,wj0,rhog,yt,j, &
!$omp& vfac,vp0,vexbsw,bstar,enerb,vparsw,dum1,vxdum,zdot,pzd0,alpha,kap,xdot,ydot,pzdot,edot, &
!$omp& new_f0,pzp,wk1,vpar,dum,qr,w4,dw,laps,ter,l)
do ith=1,nthreads
  do m=i_beg(ith),i_end(ith)
#elif OPENACC
!!$acc parallel 
!!$acc loop gang vector private(rhox,rhoy)
!$omp target data map(to:vparsp(ns,:),phincp(:),w0(:,ns),mu(:,ns),dipdr(:),dbdth(:,:),ex(:,:,0:1),xn0s(ns,:),dydr(:,:),ey(:,:,0:1),delbx(:,:,0),curvbz(:,:),psip2(:),lr(1),thfnz(:),delby(:,:,0),capns(ns,:),pif0(:,ns),gxdgy(:,:),grcgt(:,:),capts(ns,:),f(:),dpdz(:,:,0),bdcrvb(:,:),dbdr(:,:),t0s(ns,:),dadz(:,:,0),gr(:,:),qhat(:,:),q(ns),grdgt(:,:),apar(:,:,0),ez(:,:,0:1),psi(:),sf(:),bfld(:,:),nonlin(ns),psip(:),mm(ns),jfn(:),y2(:,ns),u2(:,ns),radius(:,:),mims(ns)) map(tofrom:w2(:,ns),z3(:,ns),z2(:,ns),x2(:,ns),u3(:,ns),x3(:,ns),y3(:,ns),w5(:,ns),w3(:,ns))
!$omp target teams loop private(rhox,rhoy)
  do m=1,mm(ns)
#else
  do m=1,mm(ns)
#endif 
     ...
     rhoy(2) = -rhoy(1)
     rhox(3) = 0
     rhoy(3) = rhog*(1-tor)+rhog/b/grp*fp/radiusp*qhatp*lr0/q0*grcgtp*tor+rhog/b*lr0/q0*psipp/radiusp/radiusp*grp
     rhox(4) = 0
     rhoy(4) = -rhoy(3)
...
!$omp loop bind(thread)
     do l=1,lr(1)
        ...
        yt=y2(m,ns)+rhoy(1)*cos(real(l-1)*alpha)+rhoy(3)*sin(real(l-1)*alpha)  !(rwy(1,l)+sz*rwx(1,l))*rhog
        !
        ...
        i=int(xt/dx)
        j=int(yt/dy)
        wk1=z2(m,ns)/dz-real(gclr*kcnt)
        wk0=1.0-wk1
        wi1=xt/dx-real(floor(xt/dx))
        wi0=1.0-wi1
        wj1=yt/dy-real(floor(yt/dy))
        wj0=1.0-wj1
        k=0

        exp1=exp1+wk0*(wi0*(wj0*ex(i,j,0)+wj1*ex(i,j+1,0))+wi1*(wj0*ex(i+1,j,0)+wj1*ex(i+1,j+1,0)))+wk1*(wi0*(wj0*ex(i,j,1)+wj1*ex(i,j+1,1))+wi1*(wj0*ex(i+1,j,1)+wj1*ex(i+1,j+1,1)))
        eyp=eyp+wk0*(wi0*(wj0*ey(i,j,0)+wj1*ey(i,j+1,0))+wi1*(wj0*ey(i+1,j,0)+wj1*ey(i+1,j+1,0)))+wk1*(wi0*(wj0*ey(i,j,1)+wj1*ey(i,j+1,1))+wi1*(wj0*ey(i+1,j,1)+wj1*ey(i+1,j+1,1)))
        ezp=ezp+wk0*(wi0*(wj0*ez(i,j,0)+wj1*ez(i,j+1,0))+wi1*(wj0*ez(i+1,j,0)+wj1*ez(i+1,j+1,0)))+wk1*(wi0*(wj0*ez(i,j,1)+wj1*ez(i,j+1,1))+wi1*(wj0*ez(i+1,j,1)+wj1*ez(i+1,j+1,1)))
...
!!$omp end critical
     ...
     eyp = eyp/real(lr(1))
     ezp = ezp/real(lr(1))
     delbxp = delbxp/real(lr(1))
     delbyp = delbyp/real(lr(1))
     dpdzp = dpdzp/real(lr(1))
     dadzp = dadzp/real(lr(1))
     aparp = aparp/real(lr(1))
     !
     ...
     pzdot = pzd0 + (q(ns)/mims(ns)*ezp*q0*br0/radiusp/b*psipp*grcgtp/jfnp  &
          +q(ns)/mims(ns)*(-xdot*delbyp+ydot*delbxp+zdot*dadzp))
...
#ifndef DIRECT_F
     w3(m,ns)=w2(m,ns) + 0.5*dt*(vxdum*kap + edot/ter-iorb*enerb/bfldp/bfldp*fp/radiusp*dbdtp*grcgtp*kap)*w0(m,ns)*(1.-w2(m,ns)/w0(m,ns))
#endif
     !if( (r/a .le. 0.2) .or. (r/a .ge. 0.9) )then
     !  w3(m,ns)=0.0
     !  w2(m,ns)=0.0
     !endif 
     ...
     w4=1.0-new_f0/pif0(m,ns)
     dw=w4-w5(m,ns)
     w3(m,ns)=w3(m,ns)+dw*w0(m,ns)
     w5(m,ns)=w4
#endif
  enddo
#ifdef OPENMP
...
!$omp end target data
#endif
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'ppush point 2',MPI_WTIME()
  call MPI_ALLREDUCE(mynopi,nopi(ns),1,MPI_integer, &
       MPI_SUM, MPI_COMM_WORLD,ierr)
  if(myid==0)write(*,*)'ppush point 2.0',MPI_WTIME()
  np_old=mm(ns)

#ifndef OLD_PMOVE
  call test_init_pmove(z3(:,ns),np_old,lz,ierr)
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'ppush point 2.1',MPI_WTIME()
  call test_pmove(x2(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'ppush point 2.2',MPI_WTIME()
  ...
  if (ierr.ne.0) call ppexit
  call test_pmove(w0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
#ifdef DIRECT_F 
  call test_pmove(w5(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  call test_pmove(pif0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
#endif
#else
  ...
  call pmove(w0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
#ifdef DIRECT_F
  call pmove(w5(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  call pmove(pif0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
#endif
#endif
  call end_pmove(ierr)
  mm(ns)=np_new
  !write(*,*)'myid,mm',myid,mm(ns)
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'ppush point 3',MPI_WTIME()
  !      return
...
call mpi_barrier(mpi_comm_world,ierr)
if(myid==0)then 
  write(gemout,*)'before cpush 1',mm,w3(1,ns)
  flush(gemout)
endif

!!$acc update host(x3,y2,y3,z2,z3,u2,u3,w2,w3,mu,xii,z0i,pzi,eki,u0i,w0,w5,pif0)
if(myid==0)then
  write(gemout,*)'before cpush 1',mm,w3(1,ns)
  flush(gemout)
endif
...
!!$acc parallel 
!!$acc loop gang vector private(rhox,rhoy)
!$omp target data map(to:w0(:,ns),thfnz(:),jfn(:),mm(ns),nonlin(ns),delby(:,:,0),bfld(:,:),sf(:),psi(:),ez(:,:,0:1),apar(:,:,0),grdgt(:,:),q(ns),qhat(:,:),gr(:,:),dadz(:,:,0),t0s(ns,:),dbdr(:,:),bdcrvb(:,:),dpdz(:,:,0),f(:),capts(ns,:),grcgt(:,:),gxdgy(:,:),pif0(:,ns),capns(ns,:),psip(:),lr(1),psip2(:),curvbz(:,:),delbx(:,:,0),ey(:,:,0:1),dydr(:,:),xn0s(ns,:),ex(:,:,0:1),dbdth(:,:),dipdr(:),vparsp(ns,:),radius(:,:),mu(:,ns),phincp(:),mims(ns)) map(tofrom:w3(:,ns),y2(:,ns),x2(:,ns),z3(:,ns),z2(:,ns),x3(:,ns),u2(:,ns),u3(:,ns),w5(:,ns),mypfl_es(:),y3(:,ns),w2(:,ns),mypfl_em(:),mynos,myefl_em(:),myavewi,myefl_es(:),myke)
!$omp target teams loop private(rhox,rhoy)
#endif
  do m=1,mm(ns)
     r=x3(m,ns)-0.5*lx+lr0
...
!$omp loop bind(thread)
     do l=1,lr(1)
        ...
        xs=x2(m,ns)+rhox(1)*cos(real(l-1)*alpha) !rwx(1,l)*rhog
        yt=y2(m,ns)+rhoy(1)*cos(real(l-1)*alpha)+rhoy(3)*sin(real(l-1)*alpha)
        !   BOUNDARY
        xt=mod(xs+800.*lx,lx)
        yt=modulo(yt+800.*ly,ly)
        xt = min(xt,lx-1.0e-8)
     ...
     dum = 1-w3(m,ns)/w0(m,ns)
     !if(ildu.eq.1)dum = (tgis(ns)/ter)**1.5*exp(vfac*(1/tgis(ns)-1./ter))
     !         vxdum = eyp+vpar/b*delbxp
     vxdum = (eyp/b+vpar/b*delbxp)*dum1
     w3old = w3(m,ns)
     !if(myid==0)then
     !  write(gemout,*)'cpush 1.3',w3old
     !  flush(gemout)
     !endif
#ifndef DIRECT_F
     w3(m,ns) = w2(m,ns) + dt*(vxdum*kap+edot/ter-iorb*enerb/bfldp/bfldp*fp/radiusp*dbdtp*grcgtp*kap)*w0(m,ns)*dum
#endif
     ...
     k = k+1
     !if(myid==0)then
     !  write(gemout,*)'cpush 1.41',w3old,eyp
     !  flush(gemout)
     !endif
!     !$acc atomic update
    !$omp atomic update
...
!     !$acc atomic update
    !$omp atomic update
    ! if(myid==0)then
     !  write(gemout,*)'cpush 1.411'
     !  flush(gemout)
     !endif
     mypfl_em(k)=mypfl_em(k) + w3old*(vpar*delbxp/b)
     !if(myid==0)then
     !  write(gemout,*)'cpush 1.412'
     !  flush(gemout)
     !endif
!     !$acc end atomic
    !$omp end atomic
...
!     !$acc end atomic
    !$omp end atomic
     ...
     new_f0=xnp*sqrt(1.0/ter**3)*exp(-vfac/ter)
     w4=1.0-new_f0/pif0(m,ns)
     dw=w4-w5(m,ns)
     w3(m,ns)=w3(m,ns)+dw*w0(m,ns)
     w5(m,ns)=w4
...
if(myid==0)write(*,*)'after cpush 2'
  ...
  pfltemp=rbuf(3)
  nostemp=rbuf(4)
  avewi(ns,n) = rbuf(5)/( real(tmm(1)) )/real(numprocs)
  nos(1,n)=nostemp/( real(tmm(1)) )/real(numprocs)
  ke(1,n)=ketemp/( 2.*real(tmm(1))*mims(ns) )/real(numprocs)
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     efl_es(ns,k,n)=rbuf(k)/( real(tmm(1)) )*totvol/vol(k)*cn0s(ns)/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     efl_em(ns,k,n)=rbuf(k)/( real(tmm(1)) )*totvol/vol(k)*cn0s(ns)/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     pfl_es(ns,k,n)=rbuf(k)/( real(tmm(1)) )*totvol/vol(k)*cn0s(ns)/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd
     pfl_em(ns,k,n)=rbuf(k)/( real(tmm(1)) )*totvol/vol(k)*cn0s(ns)/real(numprocs)
  end do

  !      pfl(1,n)=pfltemp/( float(tmm(1)) )
  !      efl(1,n)=mims(ns)/tets(1)*efltemp/( float(tmm(1)) )

if(myid==0)write(*,*)'after cpush 3'
  np_old=mm(ns) 
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
!!$acc update device(x3,y2,y3,z2,z3,u2,u3,w2,w3,mu,xii,z0i,pzi,eki,u0i)
#ifdef DIRECT_F
!!$acc update device(pif0,w5)
#endif
#ifndef OLD_PMOVE
  call test_init_pmove(z3(:,ns),np_old,lz,ierr)
  call test_pmove(x2(:,ns),np_old,np_new,ierr)
...
subroutine grid1(ip,n,jump_opt)
  ...
  use gem_equil
#ifdef  COUPLING
    use coupling_core_edge,only:cce_side,cce_in_weight,cce_out_weight,rhomax
    use mapping
#endif
  implicit none
  real :: phip,exp1,eyp,ezp,delbxp,delbyp,dpdzp,dadzp,aparp
  real :: enerb,vxdum,dum,xdot,ydot
  INTEGER :: m,n,i,j,k,l,ns,ip,ierr
...
if(jump_opt==0)then
  ...
  !write(*,*)'before grid1, myid=',myid,'gx_id=',gx_id
...
!$omp shared(ns,mm,x3,mu,gxdgy,w3,y3,z3,gr,qhat,q,f,grcgt,bfld,lr,mims,psip,radius,thfnz) &
!$omp shared(lx,delz,ntheta,rin,pi,dbdr,dbdth,dydr,jfn,tor,iflr,dx,lr0,dr,dth) &
!$omp shared(q0,u3,pi2,frac_inner,ly,gclr,dy,kcnt,dz) &
#ifdef COUPLING
!$omp shared(a,cce_in_weight,cce_out_weight,cce_side,rhomax) &
#endif
...
!!$acc parallel 
!!$acc loop gang vector private(rhox,rhoy)
!$omp target data map(to:thfnz(:),psip(:),mm(ns),lr(1),bfld(:,:),f(:),z3(:,ns),y3(:,ns),gxdgy(:,:),w3(:,ns),radius(:,:),gr(:,:),qhat(:,:),q(ns),grcgt(:,:),mu(:,ns),x3(:,ns),mims(ns),w3e(1:mme),w111(1:mme),x3e(1:mme),z3e(1:mme),y3e(1:mme),u3e(1:mme),w011(1:mme),w001(1:mme),w101(1:mme),w000(1:mme),w100(1:mme),w010(1:mme),w110(1:mme)) map(tofrom:myden(:,:,:),mydene(:,:,:),myupar(:,:,:))
!$omp target teams loop private(rhox,rhoy)
#endif
     do m=1,mm(ns)
...
#ifdef  COUPLING 
        if(r/a .lt. cce_in_weight .and. r/a .gt. 0.3)then
          wght=wght*real(1-cce_side)
          !write(*,*)'cce_side=0',cce_side,'r=',r/a
        elseif(r/a .gt. cce_out_weight .or. r/a .lt. 0.2)then
          wght=wght*real(cce_side)
          !write(*,*)'cce_side=1',cce_side,'r=',r/a
        elseif(r/a .ge. cce_in_weight .and. r/a .le. cce_out_weight)then
          wght=wght*((cce_out_weight-r/a)/(cce_out_weight-cce_in_weight)*real(1-cce_side)+(cce_out_weight-r/a)/(cce_out_weight-cce_in_weight)*real(cce_side))
          !write(*,*)'couping region 3','r=',r/a
        elseif(r/a .ge. 0.2 .and. r/a .le. 0.3)then
          wght=wght*(r/a-0.2)/0.1
        endif
!#endif
...
!$omp loop bind(thread)
        do l=1,lr(1)
           ...
           yt=y3(m,ns)+rhoy(1)*cos(real(l-1)*alpha)+rhoy(3)*sin(real(l-1)*alpha) !(rwy(1,l)+sz*rwx(1,l))*rhog
           xt=mod(xs+800.*lx,lx)
           if(xs .gt. lx)then
             frac_inner=0.0
           else
           ...
           myden(i,j,k) = myden(i,j,k) + wght*frac_inner
           myjpar(i,j,k) = myjpar(i,j,k)+wght*vpar*frac_inner
           mydti(i,j,k) = mydti(i,j,k)+wght*vfac*frac_inner
#else
           i=int(xt/dx)
           j=int(yt/dy)
           k=0
...
!!$acc atomic update
!$omp atomic update
           myden(i,j,0)=myden(i,j,0)+wght*frac_inner*wk0*wi0*wj0
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i+1,j,0)=myden(i+1,j,0)+wght*frac_inner*wk0*wi1*wj0
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i,j+1,0)=myden(i,j+1,0)+wght*frac_inner*wk0*wi0*wj1
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i+1,j+1,0)=myden(i+1,j+1,0)+wght*frac_inner*wk0*wi1*wj1
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i,j,1)=myden(i,j,1)+wght*frac_inner*wk1*wi0*wj0
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i+1,j,1)=myden(i+1,j,1)+wght*frac_inner*wk1*wi1*wj0
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i,j+1,1)=myden(i,j+1,1)+wght*frac_inner*wk1*wi0*wj1
!!$acc end atomic
!$omp end atomic
!!$acc atomic update
!$omp atomic update
           myden(i+1,j+1,1)=myden(i+1,j+1,1)+wght*frac_inner*wk1*wi1*wj1
!!$acc end atomic
!$omp end atomic
#endif
        enddo
...
!$omp end target data
     ...
     do i=0,im
        do j=0,jm
           do k=0,mykm
#ifndef COUPLING
              den(ns,i,j,k)=q(ns)*myden(i,j,k)/n0/jac(i,k)*cn0s(ns)!/1.32914059030377417097
#else
              den(ns,i,j,k)=q(ns)*myden(i,j,k)/n0/jac(i,k)*cn0s(ns)!/1.32914059030377417097
#endif
              den1(ns,i,j,k)=q(ns)*myden(i,j,k)/n0/jac(i,k)*cn0s(ns)
   ...
   !write(*,*)'after coupling, grid1, myid=',myid,'gx_id=',gx_id
  if(ifluid==0)return
...
!$acc end data
  ...
  do i=0,im
    if(i .le. 30)then
      dene(i,:,:)=dene(i,:,:)*0.0
    elseif(i .ge. 30 .and. i .le. 60)then
      dene(i,:,:)=dene(i,:,:)*real(i-30)/real(30)
    elseif(i .ge. 199 .and. i .le. 229)then
      dene(i,:,:)=dene(i,:,:)*real(229-i)/real(30)
    elseif( i .ge. 229)then
      dene(i,:,:)=dene(i,:,:)*0.0
    endif
  enddo
...
subroutine parperp(vpar,vperp2,m,pi,cnt,MyId)
  ...
  !r1=revers_int8(m,MyId,cnt,7)
  !r2=revers_int8(m,MyId,cnt,11)
  r1=ran(iseed2)
  r2=ran(iseed2)
...
subroutine filter(u)
  ...
  use gem_equil
  implicit none
  integer :: i,j,k,l,m,n,ip,NMDZ,NNI,ikz,ndum,ierr
  parameter(NMDZ=2)
  real :: u(0:imx,0:jmx,0:1)
...
subroutine loadi(ns)
  ...
  use gem_equil
  implicit none
  INTEGER :: i,j,k,m,idum,ns,m1,ierr
  INTEGER :: np_old,np_new
  ...
  !      ns = 1
  if(myid==0)write(*,*)'lr0,lx,rin,a',lr0,lx,rin,a
  m = 0
  do j = 1,100000000
     ...
     if(ran2(iseed)<(0.5*jacp/jacmax))then
        ...
        bfldp = wx0*wz0*bfld(i,k)+wx0*wz1*bfld(i,k+1) &
             +wx1*wz0*bfld(i+1,k)+wx1*wz1*bfld(i+1,k+1) 
        psp = wx0*psi(i)+wx1*psi(i+1)
        ter = (wx0*t0s(ns,i)+wx1*t0s(ns,i+1))*1.5
        ter_tmp=wx0*t0s(ns,i)+wx1*t0s(ns,i+1)
        ...
        xnp=wx0*xn0s(ns,i)+wx1*xn0s(ns,i+1)
        w0(m,ns)=xnp*(ter/ter_tmp)**1.5*exp(eki(m,ns)*(1/ter-1./ter_tmp))
#ifdef DIRECT_F
        pif0(m,ns)=xnp*sqrt(1./ter_tmp**3)*exp(-eki(m,ns)/ter_tmp)
        w5(m,ns)=0.0
#endif
        ...
        !w2(m,ns)=2.*amp*sin(pi2/ly*y2(m,ns))*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*exp(-(x2(m,ns)-0.4*lx)**2/(lx/8)**2)
         tmp=0.0
         do ii=1,int((jcnt-1)/2) 
           tmp=tmp+amp*cos(ii*(pi2/ly*y2(m,ns)))*exp(-0.5*((r/a-0.5)/0.07)**2)/cn0s(ns)*exp(-(z2(m,ns)-lz/2)**2/(lz/8)**2)*w0(m,ns)/xnp !rho0=0.5, simga=0.15
           !tmp=0.0
           !tmp=tmp+amp*cos(ii*(pi2/ly*y2(m,ns)))*exp(-0.5*((r/a-0.5)/0.07)**2)/cn0s(ns)*w0(m,ns)/xnp
         enddo
         !tmp=0.0
         w2(m,ns)=tmp
        if(ns==2)w2(m,ns) = 0.
...
170 continue
  ...
  avgv=avgv/real(tmm(1))/real(numprocs)
  do m=1,mm(ns)
  ...
  call pmove(w0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  !$acc update device(w0(:,ns))
#ifdef DIRECT_F
  call pmove(pif0(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  !$acc update device(pif0(:,ns))
  call pmove(w5(:,ns),np_old,np_new,ierr)
  if (ierr.ne.0) call ppexit
  !$acc update device(w5(:,ns))
#endif
  !     
  call end_pmove(ierr)
  mm(ns)=np_new
...
subroutine pint
  ...
  INTEGER :: m,i,j,k,l,n,ipover,ieover,ierr
  INTEGER :: np_old,np_new
...
subroutine cint(n)
  ...
  real :: phip,exp1,eyp,ezp,delbxp,delbyp,dgdtp,dpdzp,dadzp,aparp,vncp
  real :: dum,vxdum,dum1,dum2,eps,x,h_x,h_coll,hee,nue
  INTEGER :: m,i,j,k,l,n,mynowe,ierr
  INTEGER :: np_old,np_new
  real :: vfac,kap,kapnp,kaptp,sz,vpar,ppar,vpdum,pidum,xnp
  real :: b,th,r,enerb,cost,sint,qr,laps,ter
  real :: xt,xs,yt,zt,xdot,ydot,zdot,pzdot,edot,pzd0,pzd1,vp0
  ...
  ttrap=rbuf(6)
  ptrptmp=rbuf(7)
  avewe(n) = rbuf(8)/(real(tmm(2)) )/real(numprocs)
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     efle_es(k,n)=rbuf(k)/( real(tmm(2)) )*totvol/vol(k)*cn0e/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     efle_em(k,n)=rbuf(k)/( real(tmm(2)) )*totvol/vol(k)*cn0e/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     pfle_es(k,n)=rbuf(k)/( real(tmm(2)) )*totvol/vol(k)*cn0e/real(numprocs)
  end do
  ...
  call MPI_ALLREDUCE(sbuf,rbuf,10,  &
       ...
       MPI_COMM_WORLD,ierr)
  do k = 1,nsubd   
     pfle_em(k,n)=rbuf(k)/( real(tmm(2)) )*totvol/vol(k)*cn0e/real(numprocs)
  end do
     ...
     do l=0,im-1
        do m=0,jcnt-1
           ...
           do i=0,im-1 
              ...
              wx1 = 1.-wx0
              do ns = 1, nsm
                 ter(ns) = wx0*t0s(i1,ns)+wx1*t0s(i1+1,ns)
              enddo
              do n = 0,1
                 k = gclr*kcnt+n
                 k1 = int(k*dz/delz)
...
subroutine initialize
  ...
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after write totvol'
...
subroutine accumulate(n,ip)
...
#ifdef ADIOS2_DEBUG
  type(adios2_engine), save :: engine,engine_gem
  type(adios2_io), save :: io,io_gem
  ...
  integer, save :: init=0,init_gem=0
  write(stepid_str,'(I0.5)')n
  write(ip_str,'(I0.1)')ip
#endif

#ifndef KINETIC_ELECTRON
  ...
  call grid1(ip,n,0)
  !$acc wait
  !write(*,*)'after grid1, myid=',myid
  call mpi_barrier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'after grid1', MPI_WTIME()
#ifdef  COUPLING
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 0',MPI_WTIME()
    if(cce_side==0)then
      den_tmp=den(1,:,:,:)/q(1)
#ifndef MAP_PARALLEL
      call mapping_GEM_XGC(density_3d,pot_density_3d,den_tmp,grid,mapping_GEM_XGC_linear_coef)
#else
      call mapping_GEM_XGC_new(density_3d,pot_density_3d,den_tmp,grid,mapping_GEM_XGC_linear_coef)
#endif
    endif
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 1',MPI_WTIME()
    call cce_send_density(pot_density_3d, 0)
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 2',MPI_WTIME()
    call cce_receive_density
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 3',MPI_WTIME()
    call cce_process_density
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 4',MPI_WTIME()
    ...
    if(myid==0)then
      ...
      gdims_2d(1)=cce_all_surface_node_number
      gdims_2d(2)=nphi
      goffset_2d(1)=0
      goffset_2d(2)=0
      ldims_2d(1)=cce_all_surface_node_number
      ldims_2d(2)=nphi
     

      write(gemout,*)'start density 3d out','n=',n,'ip=',ip,'cce_all_surface_node_number'

      call flush(gemout)
      cce_filename='density_ion.3d.'//trim(stepid_str)//'.'//trim(ip_str)//'.bp'
      if(init==0)then
        call adios2_declare_io(io,adios2obj,'output_3d',ierr)
        call adios2_define_variable(var, io, "gem_data", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, ierr)
        call adios2_define_variable(var, io, "xgc_data", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, ierr)
        gdims_2d(1)=cce_all_node_number
        ldims_2d(1)=cce_all_node_number
        call adios2_define_variable(var, io, "xgc_field_data", adios2_type_dp, 2,gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, ierr)
        gdims_3d(1)=imx+1
        gdims_3d(2)=jmx+1
        gdims_3d(3)=kmx+1
        goffset_3d(1)=0
        ...
        ldims_3d(2)=jmx+1
        ldims_3d(3)=1
        call adios2_define_variable(var, io, "gem_field_data", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, ierr) 
        init=1
      endif
    endif
    if(myid==0)then
      call adios2_open(engine,io, cce_filename, adios2_mode_write, mpi_comm_self, ierr)
      call adios2_begin_step(engine, adios2_step_mode_append, ierr)
      call adios2_put(engine, "gem_data", density_3d, ierr)
      call adios2_put(engine, "xgc_data", pot_density_3d, ierr)
   ...
   if(tclr==0)then
     ...
     ldims_3d(3)=1
     if(init_gem==0)then
       call adios2_declare_io(io_gem,adios2obj,"output_3d_gem",ierr)
       call adios2_define_variable(var, io_gem, "gem_field_data", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, ierr)
       init_gem=1
     endif
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   if(myid==0)then
     write(gemout,*)'end density 3d out',' n=',n,'ip=',ip
     call flush(gemout)
   endif
#endif
  ...
  if(idg.eq.1)write(*,*)'pass grid1'
     ...
     call cce_send_field
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after send field',MPI_WTIME()
     call cce_receive_field(pot_field_3d,1)
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after receive field',pot_field_3d(63388,1),MPI_WTIME()
     if(cce_side==0)then
#ifndef MAP_PARALLEL
       call mapping_XGC_GEM(pot_field_3d,phi,field_3d,grid,mapping_XGC_GEM_linear_coef)
#else
       call mapping_XGC_GEM_new(pot_field_3d,phi,field_3d,grid,mapping_XGC_GEM_linear_coef)
#endif
     endif
     !call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after mapping XGC_GEM',phi(imx/2,jmx/2,0),MPI_WTIME()

     call cce_process_field
#ifdef ADIOS2_JYCHENG
     if(myid==0)then
       cce_filename='adi_field.3d.'//trim(stepid_str)//'.'//trim(ip_str)//'.bp'
       call adios2_open(engine,io, cce_filename, adios2_mode_write,mpi_comm_self, ierr)
       call adios2_begin_step(engine, adios2_step_mode_append, ierr)
       call adios2_put(engine, "gem_data", field_3d, ierr)
       call adios2_put(engine, "xgc_field_data", pot_field_3d, ierr)
       call adios2_close(engine, ierr)
     endif
     if(myid==0)then
       write(gemout,*)'after adi field output'
       call flush(gemout)
     endif
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)then
       write(gemout,*)'after adi field output 1'
       call flush(gemout)
     endif
     if(myid==0)write(*,*)'after process field',MPI_WTIME()
  
     if(tclr==0)then
       cce_filename='adi_gem_field.3d.'//trim(stepid_str)//'.'//trim(ip_str)//'.bp'
       call adios2_open(engine_gem, io_gem, cce_filename, adios2_mode_write, tube_comm, ierr)
       call adios2_begin_step(engine_gem, adios2_step_mode_append, ierr)
       call adios2_put(engine_gem, "gem_field_data", phi(:,:,0), ierr)
       call adios2_close(engine_gem,ierr)
       cce_filename='adi_gem_field_1.3d.'//trim(stepid_str)//'.'//trim(ip_str)//'.bp'
       call adios2_open(engine_gem, io_gem, cce_filename, adios2_mode_write,tube_comm, ierr)
       call adios2_begin_step(engine_gem, adios2_step_mode_append, ierr)
       call adios2_put(engine_gem, "gem_field_data", phi(:,:,1), ierr)
       call adios2_close(engine_gem,ierr)
     endif
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)then
       write(gemout,*)'after adi gem field output '
       call flush(gemout)
     endif
#endif
#endif
  ...
  !$acc wait
    ...
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 1',MPI_WTIME()
    call cce_send_density(pot_density_3d, 1)
    call mpi_barrier(mpi_comm_world,ierr)
    if(myid==0)write(*,*)'acc 2',MPI_WTIME()
    call cce_receive_density
    call mpi_barrier(mpi_comm_world,ierr)
...
#ifdef ADIOS2_JYCHENG
    if(myid==0)then
       cce_filename='na_density.3d.'//trim(stepid_str)//'.'//trim(ip_str)//'.bp'
       call adios2_open(engine,io, cce_filename,adios2_mode_write,mpi_comm_self, ierr)
       call adios2_begin_step(engine, adios2_step_mode_append, ierr)
       call adios2_put(engine, "gem_data", density_3d, ierr)
       call adios2_put(engine, "xgc_data", pot_density_3d, ierr)
...
#endif
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  if(myid==0)write(*,*)'end accumu', MPI_WTIME()        
...
subroutine poisson(n,ip)
...
#endif
  implicit none
  integer :: ierr,n,i,i1,j,k,ip,it,iter=1
  ...
  do it = 1,iter
     ...
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after den0_phi',MPI_WTIME()
     if(iperi==1)call gkpsL(n,ip)
#ifdef COUPLING
     if(iperi==0 .and. cce_side==1)then
#else
     if(iperi==0)then
#endif
     ...
     call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after field solver',MPI_WTIME()
#ifdef  COUPLING 
      call cce_send_field
      call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after send field',MPI_WTIME()
      call cce_receive_field(pot_field_3d,0)
      call mpi_barrier(mpi_comm_world,ierr)
     if(myid==0)write(*,*)'after receive field',pot_field_3d(63388,1),MPI_WTIME()
      if(cce_side==0)then
#ifndef MAP_PARALLEL
     ...
     if(myid==0)write(*,*)'after process field',MPI_WTIME()
#endif
     if(myid==0)write(*,*)'after field',MPI_WTIME()
  ...
  if(myid==0)write(*,*)'after rmsphi',MPI_WTIME()
...
100 continue
  ...
  !call mpi_bariier(mpi_comm_world,ierr)
  if(myid==0)write(*,*)'with filter',MPI_WTIME()
#endif
  !remove zonal field
  if(izonal==0)then
     do i=0,im-1
...
subroutine field(n,ip)
  ...
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  if(myid==0)then
    write(gemout,*)'after field',mm,phi(imx/2,jmx/2,0)
    flush(gemout)
  endif  
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine generate_3d_field(field_gem_in,field_3d_out)
  use gem_com, only:imx,jmx,kmx,grid_comm,tube_comm,gclr,tclr,glst,tlst
  use mpi
  implicit none

  integer :: ierr,icount
  real, intent(in) :: field_gem_in(0:imx,0:jmx,0:1)
  real, intent(out) :: field_3d_out(0:imx,0:jmx,0:kmx)
  real :: field_3d_out_tmp(0:imx,0:jmx,0:kmx)

  !den_tmp=den(1,:,:,:)/q(1)
  field_3d_out=0.0
  !store the density each z domain  
  field_3d_out(:,:,gclr)=field_gem_in(:,:,0)
  if(gclr==0)then
    field_3d_out(:,:,kmx)=field_gem_in(:,:,0)
  endif

  !sum of the 3d density  
  icount=(imx+1)*(jmx+1)*(kmx+1)
  call mpi_allreduce(field_3d_out,field_3d_out_tmp,icount,MPI_REAL8,mpi_sum,tube_comm,ierr)
  field_3d_out=field_3d_out_tmp

  call mpi_barrier(mpi_comm_world,ierr)

end subroutine generate_3d_field
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine push_wrapper(n,ip)
  use gem_com
  use gem_equil
  implicit none
  integer :: n,i,j,k,ip,ns,ierr

#ifdef KINETIC_ELECTRON
  integer :: sml_ncycle_half,ncycle,icycle
#endif

  do ns = 1,nsm
     if(myid==0)write(*,*)'before ppush',MPI_WTIME()
     if(ip.eq.1.and.ision==1)call ppush(n,ns)
     if(myid==0)write(*,*)'after ppush',MPI_WTIME()
     if(ip.eq.0.and.ision==1)call cpush(n,ns)
     if(myid==0)write(*,*)'after cpush',MPI_WTIME()
  end do
...
#endif
  if(ip .eq. 1)ncycle=sml_ncycle_half
  if(ip .eq. 0)ncycle=sml_ncycle_half*2
  do icycle=1,ncycle
     if(myid==0)then
       write(gemout,*)'sub-cycling, icycle=',icycle
       call flush(gemout)
     endif
#ifdef RK4
     call pushe_rk4_1step(icycle,ip,ncycle)
#else
     call pushe(icycle,ip,ncycle)
#endif
  enddo
#endif
...
13 format(1x,i6,7(2x,i7))
  if(myid.eq.master)then
     open(16, file='indicator.out', status='unknown',position='append')
...
subroutine jpar0(ip,n,it,itp)
  ...
  INTEGER :: m,n,i,j,k,l,ns,ip,it,itp,ierr
  real :: wx0,wx1,wy0,wy1,wz0,wz1,vte
...
subroutine countw(n)
  ...
  avwe = avwe/tmm(1)/numprocs
  wpbin = wpbin/tmm(1)/numprocs
...
subroutine fltx(u,isbl,ism)   
  ...
  use gem_fft_wrapper
  implicit none
  INTEGER :: i,j,k,k1,l,m,n,ifirst,nstep,ip,INFO,isbl,ism,ierr
...
subroutine dcmpy(u,v)
  ...
  use gem_fft_wrapper
  implicit none
  INTEGER :: i,j,k,k1,l,m,n,ifirst,nstep,ip,INFO,ierr
...
subroutine filtbl(u)   
  ...
  INTEGER :: i,j,k,l,m,n,ind,m1,m2,ierr
...
subroutine ldel_new
  ...
  do j = 1,100000000
     ...
     if(ran2(iseed)<(0.5*jacp/jacmax))then
        ...
        psp = wx0*psi(i)+wx1*psi(i+1)
        !ter = wx0*t0e(i)+wx1*t0e(i+1)
        ter = (wx0*t0e(i)+wx1*t0e(i+1))*1.5
        ter_tmp=wx0*t0e(i)+wx1*t0e(i+1)
        if(eldu==1)ter = tge
        b=1.-tor+tor*bfldp
        ...
        xnp=wx0*xn0e(i)+wx1*xn0e(i+1)
        !    LINEAR: perturb w(m) to get linear growth...
        !w2e(m)= 1e-10 !2.*amp*(revers(MyId*cnt+m,13)-0.5) !(ran2(iseed)
        !         w2e(m) =
        !         amp*u2e(m)/(abs(u2e(m))+0.1)*sin(x2e(m)*pi2/lx)

        w0e(m)=xnp*(ter/ter_tmp)**1.5*exp(eke(m)*(1/ter-1./ter_tmp))
#ifdef DIRECT_F
        pif0e(m)=xnp*sqrt(1./ter_tmp**3)*exp(-eke(m)/ter_tmp)
        w5e(m)=0.0
#endif

        tmp=0.0
        do ii=1,int((jcnt-1)/2)
          !tmp=tmp+amp*cos(ii*(pi2/ly*y2e(m)))*exp(-0.5*((r/a-0.5)/0.07)**2)/cn0e*exp(-(z2e(m)-lz/2)**2/(lz/8)**2)*w0e(m)/xnp
        enddo
        w2e(m)=tmp
        myavgw=myavgw+w2e(m)
...
subroutine avge
  ...
  use gem_equil
  implicit none
  INTEGER :: m,n,ip,i,j,k,j1,j2,j3,j4,ierr
  real :: bfldp,ter
  real :: wx0,wx1,wz0,wz1
...
subroutine avgi(ns)
  ...
  use gem_equil
  implicit none
  INTEGER :: ns,m,n,ip,i,j,k,j1,j2,j3,j4,ierr
  real :: bfldp,ter
  real :: wx0,wx1,wz0,wz1
  real :: r,qr,th,cost,sint,b,vfac,js,jv,dum,dum1,pidum,vel
...
subroutine rstpe
  ...
  use gem_equil
  implicit none
  INTEGER :: m,n,ip,i,j,k,j1,j2,j3,j4,j5,nobge,ierr
  ...
  do m=1,mme
     ...
     dum = totvol/tmm(1)/(dx*dy*dz*dvfac*dlamb*js*jv)/numprocs
...
subroutine ezampL(nstep,ip)   
  ...
  INTEGER :: i,j,k,i1,k1,l,m,n,ifirs,ip,nstep,ierr
  INTEGER :: l1,m1,myk
  complex :: temp3d(0:imx-1,0:jmx-1,0:1)
  real :: myrmsapa
...
subroutine ftcamp
  ...
  integer :: i,j,k,thisf,nsize,ir,ierr
...
subroutine colli(ip,n)
  ...
  integer :: i,ip,k,m,n,ncol,icol,ierr
...
#ifdef TOTAL_F
real function get_f0(xnp,ter,vfac)
  implicit none

  real :: xnp,ter,vfac

  get_f0=xnp*(ter**1.5)*exp(-vfac/ter)
